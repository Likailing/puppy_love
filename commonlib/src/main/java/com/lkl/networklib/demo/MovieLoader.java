...
import com.lkl.networklib.util.CommonLoader;
import com.lkl.networklib.util.RetrofitServiceManager;
    ...
    * */
    public Observable<List<Movie>> getMovie(int start, int count){
        return observe(apiService.getTop250Observable(start,count))
                .map(new Function<Object, List<Movie>>() {
                    @Override
                    public List<Movie> apply(Object o) throws Exception {
                        return null;
                    }
                });
    }
